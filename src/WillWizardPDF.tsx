...
export function generateWillPDF({
  user,
  contacts,
  assets,
  assetBeneficiaries,
  ...
  doc.text(
    `I, ${user.name}, of ${user.address || "________________"}, being of sound mind, declare this document to be my Last Will and Testament.`,
    20,
    y,
    { maxWidth: 170 }
  );
  ...
  if (executors.length) {
    ...
    executors.forEach((c) => {
      doc.text(
        `- ${c.name} (${c.relationship}) | Email: ${c.email} | Phone: ${c.phone}`,
        26,
        y
      );
      y += 6;
  ...
  if (guardians.length) {
    ...
    guardians.forEach((c) => {
      doc.text(
        `- ${c.name}${c.relationship ? " (" + c.relationship + ")" : ""} | Email: ${c.email} | Phone: ${c.phone}`,
        26,
        y
      );
      y += 6;
    ...
    doc.text(
      "I appoint the above-named person(s) as guardian(s) for my minor children, if any, at the time of my passing.",
      26,
      y,
      { maxWidth: 160 }
    );
  ...
  assets.forEach((asset) => {
    doc.setFont("helvetica", "bold");
    doc.text(
      `Asset: ${asset.name} (${asset.type}) - $${asset.value.toLocaleString()} [${asset.status || "Active"}]`,
      22,
      y
    );
    y += 6;
  ...
  if (beneficiaryContacts.length) {
    ...
    beneficiaryContacts.forEach((b) => {
      doc.text(
        `- ${b.name} (${b.relationship}) | Email: ${b.email} | Phone: ${b.phone}`,
        22,
        y
      );
      y += 6;
  ...
  doc.setFontSize(11);
  doc.text(
    "This will revokes all prior wills and codicils. I direct that my just debts, funeral expenses, and expenses of administering my estate be paid as soon as practical.",
    20,
    y,
    { maxWidth: 170 }
  );
  y += 14;
  doc.text(
    "IN WITNESS WHEREOF, I have signed this Will on this date:",
    20,
    y
  );
  y += 14;
